...
#heatmap of acute HIs by site and endpoint - 50th percentile values (not used in MS)
```{r}
...
#heatmap of acute HIs by site and endpoint - 95th percentile values (not used in MS)
```{r}
#get the average HI (either from drilling, pre, or prod) for each site
HI95_clean = HI95 %>%
  group_by(Sampling.Site, Endpoint) %>%
...
# Define the custom legend labels with chemical names and codes in parentheses
chemical_labels = c("Aliphatic_low" = "Aliphatic low (Al)", 
                    "Aromatic_medium" = "Aromatic medium (Am)", 
                    "Benzene" = "Benzene (B)", 
                    "Ethylbenzene" = "Ethylbenzene (E)", 
                    "n.Nonane" = "n.Nonane (N)", 
                    "Ozone" = "Ozone (O)", 
                    "PCE" = "PCE (P)", 
                    "Styrene" = "Styrene (S)", 
...
bar_immuno95_phase = ggplot(HQ95_immuno, aes(x = Phase, y = average_HQ, fill = Chemical)) +
  ...
  scale_fill_manual(values = chemical_colors, labels = chemical_labels) +  # Use custom labels for the legend
  theme_bw() +
  labs(x = "Phase", 
       y = "Average HQ", 
       fill = "Chemical(s)") +
...
#stacked bar plot of chemical-specific HQs for hematological effects for each phase (95th percentile) for O&G only
```{r}
#filter for hematological endpoint and get the average HQ of each site for each phase
HQ95_hema = HQ95 %>%
  filter(Sampling.Site == "Interchange A" | Sampling.Site == "Interchange B" | Sampling.Site == "Livingston" | Sampling.Site == "Northwest A" | Sampling.Site == "Northwest B" | Sampling.Site == "United") %>%
  filter(Endpoint == "hematological") %>%
  group_by(Phase, Chemical) %>%
  summarize(average_HQ = mean(HQ, na.rm = TRUE)) %>%
  ungroup()
...
#make stacked bar plot 
bar_hema95_phase = ggplot(HQ95_hema, aes(x = Phase, y = average_HQ, fill = Chemical)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = chemical_colors, labels = chemical_labels) +  # Use custom labels for the legend
  theme_bw() +
  labs(x = "Phase", 
...
ggsave(filename = "bar_acuteHQ95_hema_phase_OG.png", device = "png", plot = bar_hema95_phase, path = "~/CO_CRA/CO_CRA/Output/", width = 7, height = 10)
...
#get raw avg HQs for every phase and endpoint for anthem sites for reporting
```{r}
...
#stacked bar plot of chemical-specific HQs for hematological effects for each phase (95th percentile) for Anthem
```{r}
#filter for hematological endpoint and get the average HQ of each site for each phase
HQ95_hem_anthem = HQ95 %>%
  filter(Sampling.Site == "Anthem") %>%
  filter(Endpoint == "hematological")


#make stacked bar plot 
bar_hem95_phase_anthem = ggplot(HQ95_hem_anthem, aes(x = Phase, y = HQ, fill = Chemical)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = chemical_colors, labels = chemical_labels) +  # Use custom labels for the legend
  theme_bw() +
  labs(x = "Phase of Nearby Pad", 
...
bar_hem95_phase_anthem

ggsave(filename = "bar_acuteHQ95_hema_phase_anthem.png", device = "png", plot = bar_hem95_phase_anthem, path = "~/CO_CRA/CO_CRA/Output/", width = 7, height = 10)
```

## combining images for MS correction ##


#O&G site heat maps
```{r}
# Set working directory
setwd("~/CO_CRA/CO_CRA/Output")

# Define file names and labels
files <- c(
  "heat_acuteHI50_phase_OG.png",
  "heat_acuteHI95_phase_OG.png"
)

labels <- c("A", "B")

# Get max width to standardize image sizes
max_width <- max(sapply(files, function(f) {
  image_info(image_read(f))$width
}))

# Read, flatten, resize, and annotate images
...
combined <- image_append(c(imgs[[1]], imgs[[2]]))
...
# Define file names and labels
files <- c(
  "heat_acuteHI50_phase_anthem.png",
  "heat_acuteHI95_phase_anthem.png"
)
...
#O&G site chemical contributions
```{r}
# Set working directory
setwd("~/CO_CRA/CO_CRA/Output")
...
files <- c(
  "bar_acuteHQ95_dev_phase_OG.png",
  "bar_acuteHQ95_repro_phase_OG.png",
  "bar_acuteHQ95_resp_phase_OG.png",
  "bar_acuteHQ95_neuro_phase_OG.png",
  "bar_acuteHQ95_immuno_phase_OG.png",
  "bar_acuteHQ95_hema_phase_OG.png"
)
...
max_height <- max(sapply(files, function(f) image_info(image_read(f))$height))

# Read, resize, annotate
imgs <- mapply(function(file, label) {

  img <- image_read(file)
  img <- image_background(img, "white", flatten = TRUE)

  # standardize canvas so all images align perfectly
img <- image_extent(
  img,
...
image_write(
  combined,
  path = "bar_acuteHQ95_combined_OG.pdf",
  format = "pdf",
  density = "300x300"
)
...
image_write(
  combined,
  path = "bar_acuteHQ95_combined_OG.tif",
  format = "tiff",
  density = "300x300",
  compression = "lzw"
)
...
image_write(
  combined,
  path = "bar_acuteHQ95_combined_OG.png",
  format = "png"
)

#combined PDF not working in magick, trying work around (still not working - will come back if journal needs it)
library(grid)

# Convert magick object to something grid can reliably render
...
pdf("bar_acuteHQ95_combined_OG.pdf", width = 11, height = 8.5)
grid.draw(img_grob)
dev.off()
```
...
#anthem chemical contributions
```{r}
# Set working directory
setwd("~/CO_CRA/CO_CRA/Output")

# Files (3-panel Anthem set; dev removed)
files <- c(
  "bar_acuteHQ95_immuno_phase_anthem.png",
  "bar_acuteHQ95_resp_phase_anthem.png",
  "bar_acuteHQ95_hema_phase_anthem.png"
)
...
# --- SINGLE ROW (3 panels) ---
combined <- image_append(c(imgs[[1]], imgs[[2]], imgs[[3]]))
...
image_write(
  combined,
  path = "bar_acuteHQ95_combined_anthem.png",
  format = "png"
)
...
image_write(
  combined,
  path = "bar_acuteHQ95_combined_anthem.tif",
  format = "tiff",
  compression = "lzw"
)
...
pdf("bar_acuteHQ95_combined_anthem.pdf", width = 11, height = 4)
