...
#save the combined file for easy read
write.csv(all, file = "~/CO_CRA/CO_CRA/Data/weekly_VOCs_combined.csv")
```
...
for (i in 1:nrow(all)) {
  ...
  # Calculate the 50th percentile of the Arithmetic.Mean values
...
#save for QC
write.csv(all, file = "~/CO_CRA/CO_CRA/Data/weekly_VOC_ozone50.csv")
```
...
write.csv(percentiles_df, file = "~/CO_CRA/CO_CRA/Data/weekly_percentiles_df_50.csv")
...
write.csv(duration_df, file = "~/CO_CRA/CO_CRA/Output/weekly_50_forTWA.csv")
...
write.csv(twa_long, file = "~/CO_CRA/CO_CRA/Output/chronic_TWAs_50.csv")
```
