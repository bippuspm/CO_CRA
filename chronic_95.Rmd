---
title: "Chronic Risk Calculations for 95th Percentile Values"
author: "Paige Varner"
date: "2024-05-20"
...
rfcs = read.csv("~/CO_CRA/CO_CRA/Data/RfCs_April2026.csv")
...
#save for QC
write.csv(all, file = "~/CO_CRA/CO_CRA/Data/weekly_VOC_ozone95.csv")
```
...
chronic_rfcs$Chemical = gsub("Trichloroethylene (TCE)", "C2HCl3", chronic_rfcs$Chemical, fixed = TRUE)
chronic_rfcs$Chemical = gsub("Tetrachloroethylene (PCE)", "C2Cl4", chronic_rfcs$Chemical, fixed = TRUE)
chronic_rfcs$Chemical = gsub("Aliphatic low", "Aliphatic_low", chronic_rfcs$Chemical)
chronic_rfcs$Chemical = gsub("Aromatic medium", "Aromatic_medium", chronic_rfcs$Chemical)
...
percentiles_df <- all_clean %>%
  ...
  summarise_at(vars(Benzene, Ethylbenzene, Toluene, Xylenes, n.Octane, n.Nonane, n.Decane, Ozone, Aliphatic_low, Aromatic_medium, Styrene, C2HCl3, C2Cl4, Propene), ~ quantile(., 0.95, na.rm = TRUE))
...
write.csv(percentiles_df, file = "~/CO_CRA/CO_CRA/Data/percentiles_df_95.csv")
duration_df = read.csv(file = "~/CO_CRA/CO_CRA/Data/duration_df_95.csv")
...
#save duration df for QC
write.csv(duration_df, file = "~/CO_CRA/CO_CRA/Output/weekly_95_forTWA.csv")
...
#save TWA file for QC
write.csv(twa_long, file = "~/CO_CRA/CO_CRA/Output/chronic_TWAs_95.csv")
```
...
#save data
write.csv(chronic_HIs, file = "~/CO_CRA/CO_CRA/Output/chronic_TWA_HIs_95.csv")
write.csv(twa_rfcs, file = "~/CO_CRA/CO_CRA/Output/chronic_TWA_HQs_95.csv")
```
...
#save data
write.csv(chronic_HIs_VOCs, file = "~/CO_CRA/CO_CRA/Output/chronic_TWA_HIs_VOCs_95.csv")
```
...
write.csv(ozone_HQs, file = "~/CO_CRA/CO_CRA/Output/chronic_ozone_HQs_95.csv")
```
...
#format data
```{r}
#filter twa_df for just benzene, ethylbenzene, TCE, PCE and format to long
twa_cancer = twa_df %>%
...
#calculate cancer risk for each site by summing the cancer risks between benzene and EB
cancerrisk_OGduration_cum = cancerrisk_OGduration %>%
  group_by(Sampling.Site) %>%
  summarise(Total_Cancer_Risk = sum(Ind_Risk)) 

#save data 
write.csv(cancerrisk_OGduration, file = "~/CO_CRA/CO_CRA/Output/cancer_individual_95.csv")

write.csv(cancerrisk_OGduration_cum, file = "~/CO_CRA/CO_CRA/Output/cancer_cumulative_95.csv")
```
